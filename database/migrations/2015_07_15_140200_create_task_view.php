...
class CreateTaskView extends Migration {
	...
	public function up()
	{
		...
		(SELECT concat('task','','') as type,id,title,description,assigner,assignee,status,dueDate,concat(NULL,'',NULL) as minuteId from jobTasks where deleted_at IS NULL)
		UNION
		(SELECT concat('minute','','') as type,minuteTasks.id,minuteTasks.title,minuteTasks.description,minuteTasks.assigner,minuteTasks.assignee,
			...
			JOIN minutes on minuteTasks.minuteId = minutes.id JOIN meetings on minutes.meetingId = meetings.id where meetings.approved = '1' AND minuteTasks.deleted_at IS NULL)");
	}
